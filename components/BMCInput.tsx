  ...
  return (
    <div className="w-full max-w-screen-2xl mx-auto p-4 md:p-8 pb-32">
      <div className="text-center mb-12 mt-4">
        <h2 className="text-4xl md:text-6xl font-bold text-slate-800 mb-6 tracking-tight">আপনার বিজনেস মডেল ক্যানভাস</h2>
        <p className="text-2xl text-slate-600 max-w-4xl mx-auto font-light">নিচের বক্সগুলোতে আপনার ব্যবসার পরিকল্পনার বিস্তারিত লিখুন। যতটা সম্ভব পরিষ্কার এবং বিস্তারিত তথ্য দিন যাতে এনালাইসিস সঠিক হয়।</p>
      </div>
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-4 mb-5">
        <div className={`p-4 rounded-2xl transition-colors duration-300 ${highlight ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-600 group-hover:bg-indigo-50 group-hover:text-indigo-600'}`}>
          {iconMap[meta.icon]}
        </div>
        <h3 className={`font-bold text-2xl leading-tight ${highlight ? 'text-indigo-900' : 'text-slate-800'}`}>{meta.label}</h3>
      </div>
      <p className="text-base text-slate-500 mb-4 font-medium">{meta.description}</p>
      ...
      <div className="relative flex-1">
        ...
        {/* Clear Button */}
        <div className={`absolute top-2 right-2 transition-opacity duration-200 ${value ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          <button
            onClick={handleClear}
